...
class Create extends Component
{
    ...
    public function save()
    {
        ...
        $anime = Anime::create([
            ...
            'studio_id' => null, // Set null as studio is removed
            'is_premium' => $this->is_premium,
        ]);

        $anime->genres()->sync($this->selectedGenres);

        return redirect()->route('admin.anime.index')->with('message', 'Anime created successfully.');
    }
    ...
    {
        \Log::info("Anime Helper Create: Fetching info for ID {$urlId} from {$source}");
        ...
        return \Cache::remember($cacheKey, 3600, function() use ($urlId, $source) {
            try {
                switch ($source) {
                    ...
                    case 'otakudesu':
                        \Log::info("GetAnimeInfoServer: Otakudesu - urlId={$urlId}");
                        ...
                        if ($res && $res->successful()) {
                            ...
                            \Log::info("Otakudesu: Returning data - " . json_encode($result));
                            return $result;
                        } else {
